  ...
  async message (msg) {
    if (msg.content.startsWith(this.translate[this.lang]['prefix'])) {
      ...
      if (msgPrefix === this.translate[this.lang]['music']['play_prefix']) {
        ...
        if (!state) {
          this.client.voiceState[msg.guild.id] = new voicePart.VoiceState(await voice.join(), msg, this.client)
        ...
        if (/^https?:\/\/(www.youtube.com|youtube.com)\$/.test(url)) {
          state.songs.push(new voicePart.VoiceEntry(msg, url))
          if (state.firstCall) {
            state.audioPlayer()
          }
        } else {
          ...
          while (i < 5) {
            ...
            if (reaction.emoji.name === '\u2B55') {
              ...
              let entry = new voicePart.VoiceEntry(msg, urll)
        ...
        let volume = msg.content.split(' ')
        if (volume.length < 3) {
          msg.channel.send(this.textWVars('music', 'current_vol', state.current.player.volume * 100))
          return undefined
        }
        volume = msg.content.indexOf(volume[2])
